...
This repository follows a 12-week quant research build plan. The codebase implements Weeks 1-10 scope refactored into an institutional **4-Tier Architecture** (`Domain Models` → `Repositories` → `Services` → `Dataset Builders`) backed by **Neon PostgreSQL** via `psycopg` (v3) and a **FastAPI REST API**.
