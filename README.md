...
This repository covers **Weeks 1 to 9** of the 12-week implementation plan and features an institutional **4-Tier Architecture** backed by **Neon PostgreSQL** via `psycopg` (v3).

The project ingests historical equity OHLCV data from Yahoo Finance, macro series from FRED, stores both in Neon PostgreSQL with idempotent upserts, computes daily and monthly returns, builds a point-in-time universe membership table, and runs a parallel config-driven factor calculation, neutralization, evaluation, vectorized backtesting pipeline, market regime detection model, regime-adaptive multi-factor composite engine, and CVXPY Mean-CVaR portfolio risk optimization.

## Implemented Scope & Architecture

- **Institutional 4-Tier Architecture**:
  ...
  - **Layer 2 (Repositories)**: Abstract interfaces (`src/data/repositories/interfaces.py`), generic `BaseRepository` (`src/data/repositories/base.py`) using SQLAlchemy Core parameters, and concrete repositories (`PricesRepository`, `MetadataRepository`, `FundamentalsRepository`, `FactorsRepository`, `RegimesRepository`, `PortfolioRepository`, `OptimizationRepository`, `RiskRepository`).
...
- **Neon PostgreSQL Ingestion**: Production-grade connection pooling, auto-detection of pooled vs. direct connection strings, `psycopg` v3 driver, auto-batching to stay within parameter limits, and full table population (176,000+ records across 25 tables).
...
- **Week 9**: Institutional Portfolio Construction, Risk Modeling & Analytics Engine featuring pluggable covariance estimators (`BaseCovarianceEstimator` → `LedoitWolfCovariance`, `OASCovariance`, `SampleCovariance`, `FactorCovariance`), pluggable optimizers (`BasePortfolioOptimizer` → `CVaROptimizer`, `MeanVarianceOptimizer`, `RiskParityOptimizer`), modular `ConstraintManager` (position caps $\le 5\%$, sector caps $\le 20\%$, transaction cost penalization), automatic solver fallback chain (`CLARABEL` → `OSQP` → `ECOS` → `FallbackHeuristics`), standalone `analytics/` attribution engine (factor vs idiosyncratic return & risk attribution), and `experiments/` tracker logging git/config hashes for 100% reproducibility.
